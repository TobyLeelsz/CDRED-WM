...
class WorldModel(nn.Module):
	...
	def reward(self, z, a, task):
		...
		"""
		behavioral_predict, expert_predict, target_vec = self._reward(z, a)
		# pdb.set_trace()
		mean = torch.mean(target_vec, dim=0)
		b2 = torch.mean(target_vec**2, dim=0)
		bias_behavioral = self.cfg.reward_ema_coef * torch.sum((behavioral_predict - mean) ** 2, dim=1) + (1 - self.cfg.reward_ema_coef) * torch.mean(torch.sqrt(abs(behavioral_predict ** 2 - mean ** 2) / (b2 - mean ** 2)), dim=1)
		bias_expert = self.cfg.reward_ema_coef * torch.sum((expert_predict - mean) ** 2, dim=1) + (1 - self.cfg.reward_ema_coef) * torch.mean(torch.sqrt(abs(expert_predict ** 2 - mean ** 2) / (b2 - mean ** 2)), dim=1)

		return (self.cfg.expert_coef * (- self.cfg.bias_scale * bias_expert) - (1 - self.cfg.expert_coef) * (- self.cfg.bias_scale * bias_behavioral)).unsqueeze(-1)
	...
	def log_prob_expert(self, z, task, expert_action):
		...
		# eps = (expert_action - mu) / log_std.exp()
...
class CDRED(nn.Module):
	"""
	CDRED reward model with separate predictors for
	expert and behavioral data.
	"""
	def __init__(self, cfg, device="cuda"):
		...
		for i in range(self.num_target):
			self.target_network.append(layers.mlp(cfg.latent_dim + cfg.action_dim + cfg.task_dim, 2*[cfg.mlp_dim], cfg.rnd_dim).to(self.device))
		self.behavioral_predictor = layers.mlp(cfg.latent_dim + cfg.action_dim + cfg.task_dim, cfg.num_reward_layers*[cfg.reward_mlp_dim], cfg.rnd_dim).to(self.device)
		self.expert_predictor = layers.mlp(cfg.latent_dim + cfg.action_dim + cfg.task_dim, cfg.num_reward_layers*[cfg.reward_mlp_dim], cfg.rnd_dim).to(self.device)
	...
	def forward(self, z, action):
		combined_input = torch.cat((z, action), dim=1)
		target_vec = torch.zeros(self.num_target, combined_input.shape[0], self.cfg.rnd_dim).to(self.device)
		for i in range(self.num_target):
			target_vec[i, :, :] = self.target_network[i](combined_input)
		behavioral_predict = self.behavioral_predictor(combined_input)
		expert_predict = self.expert_predictor(combined_input)

		return behavioral_predict, expert_predict, target_vec

	def compute_behavioral(self, z, action):
		combined_input = torch.cat((z, action), dim=1)
		target_vec = torch.zeros(self.num_target, self.cfg.batch_size, self.cfg.rnd_dim).to(self.device)
		for i in range(self.num_target):
			target_vec[i, :, :] = self.target_network[i](combined_input)
		behavioral_predict = self.behavioral_predictor(combined_input)

		return behavioral_predict, target_vec

	def compute_expert(self, z, action):
		combined_input = torch.cat((z, action), dim=1)
		target_vec = torch.zeros(self.num_target, self.cfg.batch_size, self.cfg.rnd_dim).to(self.device)
		for i in range(self.num_target):
			target_vec[i, :, :] = self.target_network[i](combined_input)
		...
		return expert_predict, target_vec